...
class _CadetTabBarState extends State<CadetTabBar> with TickerProviderStateMixin {
  ...
  Widget _buildTabView({required int index}) {
    // LuckyDrawCampaign? _campaign;
    ...
    return RefreshIndicator(
      ...
      child: LayoutBuilder(
        builder: (ctx, constraints) {
          return ListView.separated(
            ...
            itemBuilder: (ctx, _idx) => Container(),
                separatorBuilder: (ctx, _) => const SizedBox(height: 16),
                itemCount: 1);
        }
      )
      // child: StreamBuilder<Map<int, List<LuckyWheelWinner?>>>(
      //   key: PageStorageKey("123"),
      //   stream: _bloc.winners,
      ...
      //             itemBuilder: (ctx, _idx) => _emptyState(
      //                 image: "assets/images/ic_profile_clock.png",
      //                 message: snapshot.error is String
      //                     ? snapshot.error! as String
      //                     : "Chưa có người may mắn trúng giải",
      ...
      //       if (winners[index] != null && winners[index]!.isNotEmpty) {

      //           return ListView.separated(
      //               padding: const EdgeInsets.symmetric(
      //                   horizontal: 16, vertical: 16),
      //               itemBuilder: (ctx, _idx) =>
      //                   _buildItem(winner: winners[index]![_idx]!),
      //               separatorBuilder: (ctx, _) => const SizedBox(height: 16),
      //               itemCount: winners[index]!.length);
      //       } else {
      //         return LayoutBuilder(builder: (ctx, constraints) {
      //           return ListView.separated(
      //               padding: const EdgeInsets.symmetric(
      //                   horizontal: 16, vertical: 16),
      //               itemBuilder: (ctx, _idx) => _emptyState(
      //                   image: "assets/images/ic_gift.png",
      //                   message: "Chưa có người may mắn trúng giải",
      //                   width: constraints.maxWidth,
      //                   height: constraints.maxHeight),
      //               separatorBuilder: (ctx, _) => const SizedBox(height: 16),
      //               itemCount: 1);
